...
class ZUC(object):
    ...
    def KeystreamGenerator(self, length):
        self.bit_reorganization()
        self.f()  # discard F.
        ...
        kBuffer = [work_k_g() for _ in range(length)]
        self.lfsr_work()
        return kBuffer
 
    def zuc_encrypt(self, i):
        length = len(i)
        kStream = self.KeystreamGenerator(length)
        return [inp ^ kStream[i] for i, inp in enumerate(i)]
 
 
if '__main__' == __name__:
    # Generate two all zero key and IV for encryption
    key = [0x00] * 16
    iv = [0x00] * 16
    zuc = ZUC(key, iv)
    ...
    out = zuc.zuc_encrypt(b"Go Queen's Go")
    print("Cipher text bit", ["%08x" % e for e in out])

    # Decryption
    zuc2 = ZUC(key, iv)
    out2 = zuc2.zuc_encrypt(out)
    print("Decrypted bit:", ["%08x" % e for e in out2])
    print("Decrypted text:", bytes(out2))
